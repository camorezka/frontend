...
function tickCarousel() {
  ...
  track.style.transform = "translate3d(-" + Math.round(carouselPos * 100) / 100 + "px,0,0)";
...
function startSpinAnimation(onDone, withStarItem) {
  ...
  function tick() {
    ...
    if (phase === "decel") {
      ...
      // Раскрываем победный элемент заранее — когда скорость почти нулевая (~82% пути)
      if (progress >= 0.82 && !revealDone) {
        revealDone = true;
        var winRepeat = REPEATS - 1;
