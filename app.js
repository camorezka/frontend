...
function getCarouselHalfWidth() {
  ...
  if (!track) return 0;
  // Половина = первый набор видео (totalWidth / 2)
...
// ══════════════════════════════════════════════════════════
// КАПЧА — показывается КАЖДЫЙ раз при входе
// ══════════════════════════════════════════════════════════

// Капча всегда сбрасывается — показывается при каждом заходе в мини-приложение
...
function loadStarsBalance() {
  ...
  api("/stats/" + TG_ID + "?init_data=" + encodeURIComponent(INIT_DATA))
    .then(function(stats) {
      var starsAmount = stats.stars_balance || 0;
      var el = document.getElementById("stars-amount");
      if (el) el.textContent = starsAmount;
    }).catch(function() {});
}
...
function loadInventory() {
  // Инвентарь отображается на отдельной странице inventory.html
  // Эта функция вызывается после выигрыша
}
...
function doSpin() {
  ...
  api("/spin", {
    ...
    // Обновляем баланс Stars после спина
    loadStarsBalance();
  }).catch(function(e) {
    showError(
      "Ошибка спина",
...
window.addEventListener("load", function() {
  ...
  // Капча показывается ПРИ КАЖДОМ заходе в мини-приложение
