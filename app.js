...
function forcePlayAllVideos() {
  ...
  // Также возобновляем CSS-анимацию ленты на случай если браузер её приостановил
...
setInterval(function() {
  ...
  // Убеждаемся что CSS-анимация не остановлена
  var track = document.getElementById("gifts-track");
...
window.switchTab = function(tab) {
  ...
  // Управление видимостью glass-panel
  var glassPanel = document.getElementById("glass-panel");
  var invBlock = document.getElementById("inv-block");
  if (glassPanel) {
    if (tab === "home") {
      // Показываем только если есть инвентарь
      var invVisible = invBlock && invBlock.style.display !== "none";
      glassPanel.style.display = invVisible ? "flex" : "none";
    } else {
      glassPanel.style.display = "flex";
    }
  }
};
...
function loadInventory() {
  ...
  api("/inventory/" + TG_ID + "?init_data=" + encodeURIComponent(INIT_DATA))
    .then(function(res) {
      ...
      // Refresh panel visibility if on home tab
      if (currentTab === "home") {
        var gp = document.getElementById("glass-panel");
...
function startApp() {
  ...
  api("/register", {
    ...
    // Скрываем glass-panel на главной вкладке (нет инвентаря)
    var glassPanel = document.getElementById("glass-panel");
