...
function toggleDemoMode() {
  ...
  // Кнопка одинаковая — без смайликов и изменения текста
  if (spinBtn) spinBtn.textContent = "Крутить рулетку";
...
// Показ TGS-анимации выигранного подарка (для демо и реального спина)
function showNftTgsOverlay(gift, isDemo, onClose) {
  var existing = document.getElementById("nft-video-overlay");
  if (existing) existing.remove();
  ...
  ov.id = "nft-video-overlay";
  ov.style.cssText =
    "position:fixed;inset:0;background:rgba(0,0,0,0.96);z-index:500;" +
    "display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:24px;";

  var badge = isDemo
    ? '<div style="background:linear-gradient(135deg,#7c3aed,#a855f7);color:#fff;font-family:Unbounded,sans-serif;font-size:11px;font-weight:700;letter-spacing:1.5px;padding:5px 16px;border-radius:20px;text-transform:uppercase;">Демо</div>'
    : '<div style="background:linear-gradient(135deg,#F5C518,#f0a000);color:#000;font-family:Unbounded,sans-serif;font-size:11px;font-weight:700;letter-spacing:1.5px;padding:5px 16px;border-radius:20px;text-transform:uppercase;">🎉 Выигрыш!</div>';

  var tgsId = "win-tgs-" + Date.now();
  ov.innerHTML = badge +
    '<div id="' + tgsId + '" style="width:240px;height:240px;border-radius:28px;overflow:hidden;' +
    'border:2px solid rgba(255,255,255,0.35);box-shadow:0 0 60px rgba(93,188,255,0.4);"></div>' +
    '<div style="font-family:Unbounded,sans-serif;font-size:22px;font-weight:900;color:#ffffff;text-align:center;">' +
      (gift.name || "NFT подарок") + '</div>' +
    (isDemo ? '<div style="font-size:12px;color:rgba(255,255,255,0.5);text-align:center;">Демо-режим · без реальной ставки</div>' : '') +
    '<button id="nft-ov-close" style="width:100%;max-width:320px;padding:16px;border-radius:18px;border:none;' +
    'background:linear-gradient(135deg,#5dbcff 0%,#3a9fd8 100%);' +
    'font-family:Unbounded,sans-serif;font-size:14px;font-weight:700;color:#001a2e;cursor:pointer;' +
    'box-shadow:0 4px 20px rgba(93,188,255,0.4);">' +
    (isDemo ? 'Крутить по-настоящему' : 'Забрать! 🎁') + '</button>' +
    (isDemo ? '<button id="nft-ov-demo-again" style="width:100%;max-width:320px;padding:14px;border-radius:18px;border:1px solid rgba(255,255,255,.2);' +
    'background:rgba(255,255,255,0.06);font-family:Unbounded,sans-serif;font-size:13px;' +
    'font-weight:600;color:rgba(255,255,255,0.55);cursor:pointer;">Ещё раз (демо)</button>' : '');
  ...
  // Загружаем TGS анимацию выигранного предмета
  var tgsContainer = document.getElementById(tgsId);
  if (tgsContainer && gift.src) {
    var src = gift.src || gift.tgs;
    if (src) {
      tgsContainer._tgsLoaded = false;
      tgsContainer.setAttribute('data-tgs', src);
      // Принудительно загружаем
      if (_tgsCache[src]) {
        _tgsQueue.unshift({ container: tgsContainer, src: src, json: _tgsCache[src] });
        _tgsProcessQueue();
      } else {
        fetch(src)
          ...
          .then(function(buf) {
            ...
            var json;
            try { json = JSON.parse(new TextDecoder('utf-8').decode(pako.inflate(uint8))); }
            catch(e) { json = JSON.parse(new TextDecoder('utf-8').decode(uint8)); }
            _tgsCache[src] = json;
            _tgsQueue.unshift({ container: tgsContainer, src: src, json: json });
            _tgsProcessQueue();
          })
          .catch(function() {});
      }
...
function onDemoSpin() {
  ...
  // Демо-шансы: 30% НФТ, 70% звёзды
  var showNft = Math.random() < 0.30;

  // Готовим подарок заранее, чтобы показать именно тот что выпал
  var demoWinGift = null;
  if (showNft) {
    var allGifts = [];
    NFT_TIERS.forEach(function(t) { allGifts = allGifts.concat(t.gifts); });
    demoWinGift = allGifts[Math.floor(Math.random() * allGifts.length)];
  }

  startSpinAnimation(function(winIdx, winItems) {
    demoCycleCount++;
    if (btnWrap) btnWrap.style.opacity = "";
    if (spinBtn) spinBtn.disabled = false;

    if (showNft && demoWinGift) {
      confetti();
      haptic("heavy");
      // Показываем TGS-версию выигранного подарка
      showNftTgsOverlay(demoWinGift, true, null);
    } else {
      var starCount = 1 + Math.floor(Math.random() * 5);
      showStarsEffect(starCount, null);
    }
  }, !showNft /* withStarItem: true=звёзды, false=НФТ */);
}
...
function showResult(res) {
  if (res.result === "win") {
    ...
    // Используем TGS оверлей — показываем стикер выигранного подарка
    showNftTgsOverlay({ name: res.nft_name || nftGift.name, src: nftGift.src }, false, function() {
