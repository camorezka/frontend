...
var DEMO_GIFTS = [
  { name: "Золотое кольцо",     emoji: "💍", stars: 120 },
  { name: "Криптобонус",        emoji: "🚀", stars: 500 },
  { name: "Ледяной кристалл",   emoji: "💎", stars: 80  },
  { name: "Медвежонок",         emoji: "🐻", stars: 60  },
  { name: "Мороженое NFT",      emoji: "🍦", stars: 45  },
  { name: "Молния удачи",       emoji: "⚡", stars: 200 },
  { name: "Носки удачи",        emoji: "🧦", stars: 30  },
  { name: "Суши-вечер",         emoji: "🍣", stars: 55  },
];
...
function bindDemoToggle() {
  ...
  toggle.addEventListener("change", function() {
    ...
    if (spinBtn) {
      if (demoMode) {
        spinBtn.classList.add("demo-active");
        spinBtn.textContent = "🎮 Демо-прокрутка";
      } else {
        spinBtn.classList.remove("demo-active");
...
function runDemoSpin() {
  ...
  // Показываем анимацию спина
  var existing = document.getElementById("demo-result-overlay");
  if (existing) existing.remove();

  var overlay = document.createElement("div");
  overlay.id = "demo-result-overlay";
  overlay.className = "demo-result-overlay";
  overlay.innerHTML =
    '<div class="demo-spinning-wrap">' +
      '<div class="demo-spinning-reel" id="demo-reel">🎰</div>' +
      '<div style="font-family:\'Unbounded\',sans-serif;font-size:14px;color:rgba(255,255,255,0.7)">Крутим...</div>' +
    '</div>';
  document.body.appendChild(overlay);

  // Анимируем смену эмодзи
  var emojiList = DEMO_GIFTS.map(function(g) { return g.emoji; });
  var idx = 0;
  var reel = document.getElementById("demo-reel");
  var flickInterval = setInterval(function() {
    if (reel) reel.textContent = emojiList[idx % emojiList.length];
    idx++;
  }, 80);
  ...
  setTimeout(function() {
    ...
    flickInterval = setInterval(function() {
      if (reel) reel.textContent = emojiList[idx % emojiList.length];
      idx++;
    }, 180);
  }, 800);
  ...
  setTimeout(function() {
    ...
    var gift = DEMO_GIFTS[Math.floor(Math.random() * DEMO_GIFTS.length)];
    haptic("heavy");

    overlay.innerHTML =
      '<div class="demo-result-card">' +
        '<div class="demo-result-emoji">' + gift.emoji + '</div>' +
        '<div class="demo-result-badge">ДЕМО РЕЖИМ</div>' +
        '<div class="demo-result-name">' + gift.name + '</div>' +
        '<div class="demo-result-sub">NFT подарок · ' + gift.stars + '⭐<br>В реальной игре он стал бы твоим!</div>' +
      '</div>' +
      '<button class="demo-close-btn" id="demo-close-btn">Крутить ещё</button>';
