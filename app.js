...
var ADMIN_IDS = [1693493298]; // OWNER_ID — добавь сюда tg_id допов при необходимости
...
function startSpinAnimation(onDone, withStarItem) {
  ...
  // Раскрываем победный элемент: заменяем placeholder на настоящий TGS/звёздочку.
  // Делается заранее (ещё до полной остановки), чтобы анимация успела
  // прогрузиться и проиграться к моменту, когда барабан остановится по центру.
  function revealWinItem() {
    if (revealed) return;
    revealed = true;
  ...
  function tick() {
    ...
    if (phase === "decel") {
      ...
      track.style.transform = "translate3d(-" + pos + "px,0,0)";
      // Раскрываем подарок заранее (примерно в 10 раз раньше окончания),
      // чтобы анимация успела загрузиться и проиграться до остановки
      if (!revealed && progress >= 0.9) {
        revealWinItem();
      }
      if (progress >= 1) {
        pos = target;
...
function showHomeScreen() {
  ...
  // Показываем кнопку Админ только для владельца
